...
const noise = createNoise2D(seed(directory));
...
export async function generateTubes(directory: string) {
  ...
  for (const [colorName, colorValue] of Object.entries(colors)) {
    ...
    tubeCss.push(renderTubeCSS(colorName, colorValue));
  }

  await Deno.writeTextFile(`${directory}/tubes.css`, tubeCss.join("\n\n"));
}
 ...
 */
function* iteratePath(path: Vertex[], amount: number): Iterable<Vertex> {
...
function generateVertices(
  ...
  const seedY = seedX * -1;
  return Array.from(iteratePath(path, amount)).map((v, i, { length }) => {
